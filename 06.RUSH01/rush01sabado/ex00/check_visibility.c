...
/*   Updated: 2024/11/16 20:52:05 by mvassall         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_row_visibility(t_box_map *data)
{
	...
	while (r < N)
	{
		...
		if ((visible_left != data->col_cons[r][0])
			|| (visible_right != data->col_cons[r][1]))
			return (FALSE);
		r++;
	}
	return (TRUE);
...
int	check_col_visibility(t_box_map *data)
{
	...
	while (c < N)
	{
		...
		if ((visible_top != data->row_cons[c][0])
			|| (visible_bottom != data->row_cons[c][1]))
			return (FALSE);
		c++;
	}
	return (TRUE);
